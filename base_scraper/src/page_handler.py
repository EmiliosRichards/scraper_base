        ...
        if response:
            ...
            if response.ok:
                if config.scraper_networkidle_timeout_ms > 0:
                    logger.debug(f"[RowID: {input_row_id}, Company: {company_name_or_id}] Waiting for networkidle on {url} (timeout: {config.scraper_networkidle_timeout_ms}ms)...")
                    try:
                        ...
                        logger.info(f"[RowID: {input_row_id}, Company: {company_name_or_id}] Timeout waiting for networkidle on {url} after {config.scraper_networkidle_timeout_ms}ms. Proceeding with current DOM content.")
